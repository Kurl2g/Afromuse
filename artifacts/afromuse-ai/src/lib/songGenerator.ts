...
export function formatDraftForClipboard(draft: SongDraft, genre: string, mood: string): string {
  ...
  const orderedSections = draft.diversityReport?.arrangementOrder?.length
    ? draft.diversityReport.arrangementOrder
        .map((key) => sectionMap[key])
        .filter((section): section is { label: string; lines: string[] } => Boolean(section?.lines?.length))
    : [
        ...(draft.intro && draft.intro.length > 0 ? [sectionMap.intro] : []),
        sectionMap.hook,
        sectionMap.verse1,
        ...(draft.verse2 && draft.verse2.length > 0 ? [sectionMap.verse2] : []),
        ...(draft.bridge && draft.bridge.length > 0 ? [sectionMap.bridge] : []),
        ...(draft.outro && draft.outro.length > 0 ? [sectionMap.outro] : []),
      ].filter((section): section is { label: string; lines: string[] } => Boolean(section?.lines?.length));
  ...
  for (const section of orderedSections) {
    sections.push(`[ ${section.label} ]`, ...section.lines, "");
  }
